...
- Phase 5 generation code commit: recorded as `phase5_code_commit` in `results/phase5/manifests/reproducibility_manifest.json`
- Phase 5 generation source hash: recorded as `phase5_source_code_hash` in `results/phase5/manifests/reproducibility_manifest.json`
- Phase 5 generation Git state: recorded as `phase5_git_dirty`; formal Phase 5 artifacts require `false`
...
The Phase 4 formal training commit and the Phase 5 plotting/table-generation commit are separate metadata fields. Phase 5 does not retrain TD3, DDPG, SAC, or any baseline; it only validates, normalizes, analyzes, and plots the already generated results.
...
To regenerate Phase 5 from a specific code version:

```bash
git checkout <phase5_code_commit>
...
Formal generation checks the relevant Git status before writing artifacts. Uncommitted changes in Phase 5 scripts or configs cause generation to stop. Changes under `results/phase5/` are ignored during this check because those files are generated outputs.

Figure 1 is generated by `scripts/plot_paper_figures.py` from `configs/comm_env_default.yaml`. It draws H, R, and L UAV positions, the H-R and R-L links, relay flight bounds, horizontal projection references, distance labels `d_HR` and `d_RL`, and elevation angle labels `theta_HR` and `theta_RL`.
...
must match the formal Phase 4 training metadata. The manifest fields `phase5_code_commit`
and `phase5_source_code_hash` identify the committed Phase 5 generation code.
...
Then compare each line in `results/phase5/manifests/file_checksums.sha256` against the
corresponding repository-relative file. The checksum file covers configs, Phase 4 CSV
and JSON metadata, Phase 5 tables, figures, and analysis files, including Figure 1.
