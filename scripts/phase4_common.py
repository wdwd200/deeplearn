...
def train_algorithm_seed(
    ...
    for episode in range(1, int(training["episodes"]) + 1):
        ...
        if episode % int(training["eval_interval"]) == 0 or episode == int(training["episodes"]):
            eval_metrics = run_agent_episode(
                agent,
                algorithm,
                env_config,
            ...
            current_eval_row = eval_row(episode, eval_metrics)
    ...
    save_json(output_path / "training_params.json", dict(config))
...
def evaluate_saved_agent(
    ...
    env_config = env_config if env_config is not None else load_config(COMM_CONFIG_PATH)
    agent = load_agent_from_dir(algorithm, model_dir, env_config=env_config, prefer_best=True)
    metrics = run_agent_episode(
        agent,
        algorithm,
        env_config,
        seed=30_000 + int(training_seed) * 100 + int(evaluation_episode),
        max_steps=max_steps,
        explore=False,
        action_transform=action_transform,
    )
...
def evaluate_policy_episode(policy: Any, evaluation_episode: int, max_steps: int) -> dict[str, Any]:
    env = UAVRelayCommEnv(config=policy.config)
    observation, info = env.reset(seed=40_000 + evaluation_episode)
    metrics = EpisodeMetrics()
    steps = 0
    while True:
...
def baseline_episode_rows(config: Mapping[str, Any], env_config: EnvConfig | None = None) -> list[dict[str, Any]]:
    experiment = config["experiment"]
    env_config = env_config if env_config is not None else load_config(COMM_CONFIG_PATH)
    rows: list[dict[str, Any]] = []
    for policy in make_default_policies(env_config, seed=0):
        for episode in range(int(experiment["evaluation_episodes"])):
            rows.append(evaluate_policy_episode(policy, episode, max_steps=int(experiment["max_steps"])))
    return rows
